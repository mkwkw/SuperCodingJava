...
// 2. 주문 아이템을 만듭니다.
        ...
        orders.add(new Order(1001, customer1, "PAID", orderItems1));
        orders.add(new Order(1002, null, "SHIPPED", orderItems2));
        orders.add(new Order(1003, customer3, "SHIPPED", orderItems2));
        orders.add(new Order(1004, customer2, "ON ORDER", null));
        orders.add(new Order(1005, customer4, "NOT PAID", null));
        orders.add(new Order(1006, customer1, "SHIPPED", orderItems2));
        ...
        for(Order order: orders){
            try {
                ...
                order.getCustomer().orElseThrow(() -> {throw new RuntimeException("고객이 누락 되었습니다.");});
                if (order.getOrderStatus().equals("주문 처리 중" )) throw new RuntimeException(order.getCustomer() + "님의 주문이 아직 주문 처리 중입니다.");
                order.getOrderItems().orElseThrow(() -> {throw new RuntimeException(order.getCustomer() +"님의 주문 아이템들이 누락 되었습니다.");});

                totalOrderCount++;

                for (OrderItem orderItem: order.getOrderItems().get()){
