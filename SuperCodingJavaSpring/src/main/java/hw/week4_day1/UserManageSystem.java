    ...
    public static void main(String[] args){
        ...
        for(User user: users){
            if (user instanceof CustomerRole){
                ((CustomerRole) user).order(user.username);
                ((CustomerRole) user).payment(user.username);
            }
            ...
            if (user instanceof AdminRole){
                ((AdminRole) user).manageUsers(user.username);
                ((AdminRole) user).changeSettings(user.username);
            }
        }
    }
}
